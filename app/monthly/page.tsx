...
function Rows({ rows }: { rows: MonthlyBatter[] }) {
  return <ol className="space-y-2">{rows.map((row, index) => {
    ...
    return <li key={`${row.batter.teamId}-${row.batter.rank}`}><Link href={`/year/${row.batter.year}/${row.batter.rank}`} style={{ borderLeftColor: color.bg, backgroundColor: withAlpha(color.bg, 0.07) }} className="flex items-center gap-3 rounded-xl border border-l-[5px] border-zinc-200/80 px-3 py-2.5 transition-transform hover:-translate-y-0.5 hover:shadow-sm">
      ...
      <span className="min-w-0 flex-1"><span className="block truncate text-base font-bold tracking-tight text-zinc-900 sm:text-lg">{row.batter.name}</span><span className="mt-0.5 flex flex-wrap items-center gap-1.5"><span style={{ backgroundColor: withAlpha(color.bg, 0.16), color: color.bg }} className="rounded-full px-1.5 py-0.5 text-[10px] font-bold">{row.batter.teamName}</span><span className="text-[10px] font-medium text-zinc-400 sm:text-[11px]">{row.hr}本・{row.rbi}打点・{row.pa}/{row.requiredPa}打席</span></span></span>
      <span className="shrink-0 text-right"><span className="block text-xl font-extrabold tabular-nums text-zinc-950 sm:text-2xl">{fmtRate(row.ops)}</span><span className="block text-[10px] font-medium text-zinc-400">OPS</span></span>
    </Link></li>;
  })}</ol>;
}

function LeagueSection({ title, month, rows }: { title: string; month: number; rows: MonthlyBatter[] }) {
  return <section className="rounded-xl border border-zinc-200 bg-white p-4"><div className="mb-3 flex items-baseline justify-between gap-3"><h2 className="text-base font-bold tracking-tight text-zinc-900">{title}</h2><span className="text-[11px] text-zinc-400">{month}月・月間規定打席</span></div><Rows rows={rows} /></section>;
}
...
export default async function MonthlyPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:py-10">
    ...
    {monthly && (monthly.central.length > 0 || monthly.pacific.length > 0) ? <div className="grid gap-4 lg:grid-cols-2"><LeagueSection title="セ・リーグ 月間OPS TOP10" month={monthly.period.month} rows={monthly.central} /><LeagueSection title="パ・リーグ 月間OPS TOP10" month={monthly.period.month} rows={monthly.pacific} /></div> : <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm leading-relaxed text-zinc-600"><p className="font-bold text-zinc-800">{monthly?.period.label ?? "今月"}は、まだ月間規定打席に到達した打者がいません。</p><p className="mt-1">試合後のデータがたまると、この月のランキングを表示します。過去月は上のボタンから確認できます。</p></div>}
  </div>;
}
