      ...
      {rows.map((row, index) => {
        ...
        return (
          <li key={`${row.batter.teamId}-${row.batter.rank}`}>
            <Link
              href={`/year/${row.batter.year}/${row.batter.rank}?from=monthly`}
              style={{
                borderLeftColor: color.bg,
                backgroundColor: withAlpha(color.bg, 0.1),
              }}
              className="flex items-center gap-3 rounded-xl border border-l-[6px] border-zinc-200/80 px-3 py-2.5 transition-transform hover:-translate-y-0.5 hover:shadow-sm"
            >
              <span
                style={{
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-1 flex flex-wrap items-center gap-1.5">
                  <TeamBadge teamId={row.batter.teamId} name={row.batter.teamName} />
              ...
              <span className="shrink-0 text-right">
                <span className="block text-xl font-extrabold leading-none tabular-nums text-zinc-950 sm:text-2xl">
                  {fmtRate(row.ops)}
                </span>
...
export default async function MonthlyPage({ searchParams }: { searchParams: Promise<{ month?: string }> }) {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:py-10">
    ...
    {monthly?.label && <p className="mt-4 text-[11px] leading-relaxed text-zinc-500">集計期間：{monthly.label}。{monthly.coverageNote}を基準とし、球団試合数 × 3.1打席以上の打者を表示しています。</p>}
  </div>;
}
